...
<<formats, Formats:>>:: Technische Formate, wie MS-Word (docx), PDF, HTML oder andere.

<<toolchains, Toolchains:>>:: Tools und Skripte, die +arc42+ anwenden oder <<formats>>/<<versions>> erzeugen (z. B. Groovy, Shell, Pandoc)
...
Contributor:: Unterstützt und verbessert <<formats, template formats>>, <<versions>>, <<toolchains, toolchains>> und/oder Inhalte. Meldet Fehler oder trägt zu Übersetzungen bei.
...
* Die Generierung der Formate soll so weit wie möglich automatisiert sein
* Der Build der <<out_formats, Ausgabeformate>> erfolgt mit Groovy-Skripten und Shell (z. B. `build-arc42.sh`)
* Weitere Tools (z. B. Pandoc) werden automatisiert installiert und genutzt
